...
let actionHistory = [];
let agentMemory = [];
let globalAgentPlan = ""; // 🧠 Structured mission plan generated by the Planner at task start
...
function DOMScanner(showOverlays) {
    ...
    function getUniqueSelector(el) {
        ...
        // 🧠 V8: Prefer stable, unique attributes over brittle positional chains
        ...
        for (const attr of stableAttrs) {
            ...
            if (val) {
                const sel = el.nodeName.toLowerCase() + '[' + attr + '="' + val.replace(/"/g, '\\"') + '"]';
                try { if (document.querySelectorAll(sel).length === 1) return sel; } catch (e) { }
            }
        }
        // Fallback: positional path, anchored at the nearest uniquely-identified ancestor
        let path = []; let current = el;
        while (current && current.nodeType === Node.ELEMENT_NODE) {
            if (current.id && document.querySelectorAll('#' + esc(current.id)).length === 1) {
                path.unshift('#' + esc(current.id));
                break;
            }
...
function parseAgentJSON(text) {
    ...
    // Repair pass 1: extract the first {...} block
    const start = cleaned.indexOf("{");
    const end = cleaned.lastIndexOf("}");
    if (start !== -1 && end > start) {
        let candidate = cleaned.substring(start, end + 1);
        try { return JSON.parse(candidate); } catch (e) { }
        // Repair pass 2: strip trailing commas and raw newlines inside the object
        candidate = candidate.replace(/,\s*([}\]])/g, "$1").replace(/\r?\n/g, " ");
        try { return JSON.parse(candidate); } catch (e) { }
    }
    throw new SyntaxError("Unparseable LLM response: " + cleaned.substring(0, 120));
}

// 🧠 V8: Ask the Planner model for an explicit step-by-step mission plan (Claude Code style)
async function generateMissionPlan(goal, apiKey, modelName, provider, baseUrl, temperature) {
...
MISSION PLAN (follow step-by-step; adapt and re-plan if a step fails):
${globalAgentPlan || "(no explicit plan — classify the task yourself)"}
RECENT ACTIONS: ${history.slice(-20).join(" | ")}
SAVED MEMORY (${memory.length} total, latest 40 shown): ${memory.slice(-40).join(" | ")}
...
CRITICAL OPSEC: DO NOT add any extra keys! DO NOT write arrays! Output ONLY this exact 5-property JSON object!`;
    ...
    while (retries > 0) {
        try {
            ...
            const backoffMs = 2000 * Math.pow(2, 3 - retries); // 🧠 V8: 2s, 4s, 8s exponential backoff
            console.log(`[NanoAgent] Retrying LLM call in ${backoffMs / 1000}s... (${retries} retries left)`);
